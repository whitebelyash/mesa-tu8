...
{
   /* ========== A8XX: ПОВЫШЕННЫЕ ЛИМИТЫ ПРОИЗВОДИТЕЛЬНОСТИ ========== */
   bool is_gen8 = pdevice->info->chip >= A8XX;
   size_t max_desc = is_gen8 ? 1024 : max_descriptor_set_size;
   uint32_t max_samplers = is_gen8 ? 512 : max_descriptor_set_size;
   uint32_t max_bound_sets = is_gen8 ? 64 : pdevice->usable_sets;
   uint32_t max_memory_alloc_count = is_gen8 ? 8192 : UINT32_MAX;
   uint32_t max_sampler_alloc_count = is_gen8 ? 8192 : 64 * 1024;
   /* =============================================================== */
   ...
   props->maxMemoryAllocationCount = max_memory_alloc_count;
   props->maxSamplerAllocationCount = max_sampler_alloc_count;
   props->bufferImageGranularity = 64;          /* A cache line */
   props->sparseAddressSpaceSize = pdevice->va_size;
   props->maxBoundDescriptorSets = max_bound_sets;
   props->maxPerStageDescriptorSamplers = max_desc;
   props->maxPerStageDescriptorUniformBuffers = max_desc;
   props->maxPerStageDescriptorStorageBuffers = max_desc;
   props->maxPerStageDescriptorSampledImages = max_desc;
   props->maxPerStageDescriptorStorageImages = max_desc;
   props->maxPerStageDescriptorInputAttachments = MAX_RTS;
   props->maxPerStageResources = max_desc;
   props->maxDescriptorSetSamplers = max_samplers;
   props->maxDescriptorSetUniformBuffers = max_desc;
   props->maxDescriptorSetUniformBuffersDynamic = MAX_DYNAMIC_UNIFORM_BUFFERS;
   props->maxDescriptorSetStorageBuffers = max_desc;
   props->maxDescriptorSetStorageBuffersDynamic = MAX_DYNAMIC_STORAGE_BUFFERS;
   props->maxDescriptorSetSampledImages = max_desc;
   props->maxDescriptorSetStorageImages = max_desc;
   ...
   props->maxFragmentCombinedOutputResources = MAX_RTS + max_desc * 2;
   ...
   props->maxEmbeddedImmutableSamplers = max_desc;
   ...
   props->maxPerStageDescriptorAccelerationStructures = max_desc;
   props->maxPerStageDescriptorUpdateAfterBindAccelerationStructures = max_desc;
   props->maxDescriptorSetAccelerationStructures = max_desc;
   props->maxDescriptorSetUpdateAfterBindAccelerationStructures = max_desc;
...
{
   /* A8XX: все 4 уровня приоритета для 8-го поколения */
   if (pdevice->info->chip >= A8XX) {
                ...
                VkDevice *pDevice)
{
   ...
   /* A8XX: увеличенный suballocator для лучшей производительности */
   uint32_t suballoc_size = 256 * 1024;
   if (device->physical_device->dev_id.gpu_id == 810) {
      suballoc_size = 512 * 1024;
   }
   ...
   if (device->physical_device->dev_id.gpu_id == 810) {
      device->vsc_draw_strm_pitch = 0x2000 + VSC_PAD;
      device->vsc_prim_strm_pitch = 0x2000 + VSC_PAD;
   } else {
